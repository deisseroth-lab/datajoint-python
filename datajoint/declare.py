...
TYPE_PATTERN = {k: re.compile(v, re.I) for k, v in dict(
    ...
    UUID=r'uuid$',
    ADAPTED=r'<.+>$',
    FILEREF=r'fileref@(?P<store>[a-z]\w*)$'
).items()}

# custom types are stored in attribute comment
SPECIAL_TYPES = {'UUID', 'INTERNAL_ATTACH', 'EXTERNAL_ATTACH', 'EXTERNAL_BLOB', 'FILEPATH', 'ADAPTED', 'FILEREF'}
NATIVE_TYPES = set(TYPE_PATTERN) - SPECIAL_TYPES
EXTERNAL_TYPES = {'EXTERNAL_ATTACH', 'EXTERNAL_BLOB', 'FILEPATH', 'FILEREF'}  # data referenced by a UUID in external tables
